          ...
          if (res.result[0] <= 1) {
            ...
            res.result[1].forEach((stock) => {
              if (stock.change) {
                ...
                stock_table.innerHTML += `<tr><td><a title="${stock.title}" href="https://finance.yahoo.com/quote/${stock.symbol}">${stock.symbol}</a></td><td>${stock.change}</td><td>$${stock.marketValue ? Math.round(stock.marketValue) : 0}</td></tr>`;
                ++changes;
              }
            });
            ...
            res.result[1].forEach((stock) => {
              stock_table.innerHTML += `<tr><td><a title="${stock.title}" href="https://finance.yahoo.com/quote/${stock.symbol}">${stock.symbol}</a></td><td>${Math.round(stock.target)}%</td><td>$${stock.marketValue ? Math.round(stock.marketValue) : 0}</td></tr>`;
            });
          }
        }
      });
